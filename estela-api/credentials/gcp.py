...
class GCPCredentials(Credentials):
    def __init__(self):
        credentials, _ = google.auth.load_credentials_from_dict(
            json.loads(settings.GOOGLE_APPLICATION_CREDENTIALS)
        )
        ...
        ] = settings.GOOGLE_APPLICATION_LOCATION
    ...
    def download_project(self, bucket_name, project_name):
        client = storage.Client(
            credentials=self.auth_credential,
            project=self.credentials["GOOGLE_APPLICATION_PROJECT_ID"],
        )
