...
build_client = cloudbuild_v1.services.cloud_build.CloudBuildClient(
    credentials=GCP.auth_credential
)
...
def build_image(estela_project_path, dockerfile, project_zip, image):
    ...
    build.source.storage_source = cloudbuild_v1.StorageSource(
        bucket="estela-api-projects", object=project_zip
    )
    build.steps.append(
    ...
    operation = build_client.create_build(
        project_id=GCP.credentials["GOOGLE_APPLICATION_PROJECT_ID"], build=build
    )

    result = operation.result()


def get_spiders_from_logs(execution_id):
    # Instantiates a client
    client = google.cloud.logging.Client(
        credentials=GCP.auth_credential,
        project=GCP.credentials["GOOGLE_APPLICATION_PROJECT_ID"],
    )
    filter_str = f'labels."run.googleapis.com/execution_name"="{execution_id}"'
    for entry in client.list_entries(
        filter_=filter_str, order_by=DESCENDING
    ):  # API call(s)
...
def init_image(project_id, location, job_id, image):
    ...
    job = run_v2.Job(template=template)
    request = run_v2.CreateJobRequest(
        parent=f"projects/{project_id}/locations/{location}", job_id=job_id, job=job
    )
    operation = run_client.create_job(request=request)
    result = operation.result()


def execute_image(project_id, location, job_id):  # Run the created job
...
def main():
    ...
    try:
        ...
        build_image(
            estela_project_path=PROJECT_PATH,
            dockerfile=DOCKERFILE_PATH,
            project_zip=project_zip_name,
            image=ESTELA_IMAGE,
        )
        ...
        spiders = get_spiders(
            project_id=GCP.credentials["GOOGLE_APPLICATION_PROJECT_ID"],
            location=GCP.credentials["GOOGLE_APPLICATION_LOCATION"],
            job_id=PID,
            image=ESTELA_IMAGE,
        )
